...
impl Quadtree {
    ...
    pub fn check_collisions(
        &self,
        collisions: &mut Vec<(Collidable, Collidable, usize, Vector2<f32>)>,
    ) -> Vec<&Collidable> {
...
pub fn possible_collision_between(a: &Collidable, b: &Collidable, collisions: &mut Vec<Collision>) {
    match a.collidable_shape {
        CollidableShape::Circle => match b.collidable_shape {
            CollidableShape::Circle => {
                if dist_circ_circ(a.centrex, a.centrey, b.centrex, b.centrey) < a.radius + b.radius
                {
                    ...
                    let delta = bpos - apos;
                    let dunit = if delta.norm() > 0. {
                        nalgebra::normalize(&delta)
                    } else {
                        nalgebra::zero()
                    };
            ...
            CollidableShape::Polygon => {
                match dist_poly_circ_with_face_and_supp(
                    b.nvert, &b.vertx, &b.verty, a.centrex, a.centrey,
                ) {
                    (d, f, s) => {
                        if d <= a.radius
                            || poly_contains_pnt(b.nvert, &b.vertx, &b.verty, a.centrex, a.centrey)
                        {
        ...
        CollidableShape::Polygon => match b.collidable_shape {
            CollidableShape::Circle => {
                match dist_poly_circ_with_face_and_supp(
                    a.nvert, &a.vertx, &a.verty, b.centrex, b.centrey,
                ) {
                    (d, f, s) => {
                        if d <= b.radius
                            || poly_contains_pnt(a.nvert, &a.vertx, &a.verty, b.centrex, b.centrey)
                        {
            ...
            CollidableShape::Polygon => {
                match penetration(
                    a.nvert, &a.vertx, &a.verty, &a.normx, &a.normy, b.nvert, &b.vertx, &b.verty,
                ) {
                    (da, fa, sa) => {
                        match penetration(
                            b.nvert, &b.vertx, &b.verty, &b.normx, &b.normy, a.nvert, &a.vertx,
                            &a.verty,
                        ) {
                            (db, fb, sb) => {
                                if da < 0. && db < 0. {
